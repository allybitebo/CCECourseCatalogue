...
class User{
	...
	protected static $tbl_name = "staffs";
	function db_fields(){
		global $mydb;
	 ...
	 static function AuthenticateUser($email="", $h_upass=""){
		...
		$row_count = $mydb->num_rows($cur);//get the number of count
		 ...
		 $found_user = $mydb->loadSingleResult();
		    $_SESSION['ACCOUNT_ID'] 	 = $found_user->staff_id;
            $_SESSION['ACCOUNT_NAME']    = $found_user->dname;
            $_SESSION['ACCOUNT_USERNAME']= $found_user->email;
            $_SESSION['ACCOUNT_PASSWORD']= $found_user->password;
            $_SESSION['ACCOUNT_TYPE']    = $found_user->level;
        	return true;
			}else{
				return false;
			}	
	...
	function single_user($id=0){
			global $mydb;
			$mydb->setQuery("SELECT * FROM ".self::$tbl_name." Where staff_id= {$id} LIMIT 1");
