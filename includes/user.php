...
class Student {
	...
	function single_user($id=0){
			global $mydb;
			$mydb->setQuery("SELECT * FROM ".self::$tbl_name." Where ACCOUNT_USERNAME= {$id} LIMIT 1");
	...
	static function Authenticatestudent($idno=""){
		...
		$mydb->setQuery("SELECT * 
						FROM  `useraccounts` 
						WHERE  `ACCOUNT_USERNAME` ='{$idno}'" );
		$cur = $mydb->executeQuery();
		$row_count = $mydb->num_rows($cur);//get the number of count
		 if ($row_count == 1){
		 $found_user = $mydb->loadSingleResult();
		 
		    $_SESSION['ACCOUNT_USERNAME'] 	 = $found_user->IDNO;
            $_SESSION['ACCOUNT_NAME']   = $found_user->ANAME;
        	return true;
			}else{
				return false;
			}	
